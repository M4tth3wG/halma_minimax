...
def minimax(node, depth, maximizing_player, heuristic):
    ...
    if maximizing_player == current_player:
        ...
        for child in children:
            move_graded = max(move_graded, minimax(child, depth - 1, maximizing_player, heuristic), key=lambda x: x[0])
        return move_graded
    else:
        move_graded = (float('inf'), [])
        for child in children:
            move_graded = min(move_graded, minimax(child, depth - 1, maximizing_player, heuristic), key=lambda x: x[0])
        return move_graded
...
class MinimaxStrategy(HalmaStrategy):
    ...
    def move(self, halma_game):
        ...
        grade, best_move = minimax(start_node, self.depth, halma_game.current_player, self.heuristic)
